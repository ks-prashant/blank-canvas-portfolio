...
### Step 4 — Port the surviving landing sections
...
**Done when:** the landing page renders every ported section from generated content, nothing hand-typed, and diffs against the prototype only where §11.1 says it should.

---

### Step 5 — The new hero and the deepened lens

**Owner: Claude Code.** The first genuinely new landing work.

- `HeroCounterEntry` (C9) replaces the prototype's retired mark row — ~12 paired marks, ink above / red below, the D9 thesis line beneath (§17.3, `DESIGN-PLAN.md` §6).
- `LensPicker` reframed as the site's one clarifying question, **with the honest un-lensed default state** (§4.1) — this is new; the prototype pre-selects a lens.
- `LensReceipt` (C22), `UnlensedMark` (C23), and the three `LensSignature` visuals (C8) — all new (§8.6).
- The two `LiveProductEntry` rows rewritten for the current products.

**Done when:** switching lens visibly re-argues the page (order + signature visual + copy), the un-lensed state renders the generalist read, and acceptance criterion 14 passes.
...
## 21. First three actions — as of 2026-07-19
...
1. **Commit the moved files and push**, confirm the commit appears in Lovable's sandbox. Closes Step 1.
2. **Port the design system out of the prototype** (§11 step 2) — tokens, fonts, grain, motion, layout shell only, never its stale copy. The step that proves the prototype is being extended, not replaced.
3. **Run §12.4's content pipeline** (§11 step 3) — the content pipeline is the critical path; every component from step 4 onward depends on its output existing.
