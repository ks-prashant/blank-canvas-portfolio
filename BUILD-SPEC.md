...
| 7 — Case pages + ship | 🟡 In progress | **Six case pages done**, **§13 a11y/responsive audit done** (real fixes: contrast, skip-link, `<main>` landmark, 320px overflow, touch targets, tabular-nums — see prose below). **Deploy not started** — next up, pending explicit go-ahead since it's a shared/visible action. |
...
**Remaining to close Step 7 and V1: deploy.** Per the deploy procedure above (push → confirm sandbox SHA → deploy → verify live SHA), the push and sandbox-sync halves are done as of this commit. The actual `deploy_project` call is a shared, visible action (it publishes a real, reachable URL) — paused for explicit go-ahead, same pause-point convention as every commit/push in this build.
