...
### 00 · Recruiter
> Nine years across five industries. Most recently I owned the entire AI product suite at a recruitment platform serving 150+ recruiters and 2,500+ client companies — 140% more work per recruiter, hiring ~25% faster. I've also shipped two AI products solo, outside any job, both live right now.
...
### 00 · Engineer
> **AI Product Manager who writes the backend.** Two live systems, built solo: a Retell voice agent made loop-proof by structure — the commitment ladder has no upward transition, so a declined ask can't be re-offered — and a governance assistant whose post-generation check drops any claim its source doesn't support. Same move both times: when an instruction can drift, replace it with a structure that can't.
